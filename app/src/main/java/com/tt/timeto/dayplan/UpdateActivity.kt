...
class UpdateActivity : AppCompatActivity(), TimePickerDialog.OnTimeSetListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val notificationList: List<Notification>? = AppDatabase.getDatabase(applicationContext)?.notificationDao()
        ...
        if (notificationList?.size!! >= 1) {
            calendar.timeInMillis = notificationList[0].reservedTime
        ...
        alarmCancelBtn.setOnClickListener {
            // 알람 취소 함수
            var notifications: List<Notification>? = AppDatabase.getDatabase(applicationContext)?.notificationDao()?.getNotificationByToDo(uToDoId.toLong())
            cancelAlarm(notifications)
        }
    }

    private fun cancelAlarm(notifications: List<Notification>?) {
